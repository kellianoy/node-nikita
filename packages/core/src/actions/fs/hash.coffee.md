...
If the `target` property is a directory, the returned hash is the sum of all the
hashs of the files it recursively contains. The default algorithm to compute the
hash is md5.

If `target` is a link, the returned hash is of the linked file.
...
* `hash`   
  The hash of the file or directory identified by the `target` property.
